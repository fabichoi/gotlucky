...
func (r *GameResultRepository) FindAllBySessionID(sessionID uint) ([]model.GameResult, error) {
	...
	err := r.db.Preload("User").Where("game_session_id = ?", sessionID).Find(&gameResults).Error
...
func (r *GameResultRepository) FindByID(sessionID, gameResultID uint) (*model.GameResult, error) {
	...
	err := r.db.Preload("User").Where("game_session_id = ? AND id = ?", sessionID, gameResultID).First(&p).Error
	return &p, err
}
