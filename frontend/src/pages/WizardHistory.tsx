...
import { fetchWizardHistory, deleteWizardGame } from "../api/gameApi";
import LoadingSpinner from "../components/LoadingSpinner";
import { useUser } from "../context/UserContext";
import { WizardGame } from "../types/wizard";
import { calculateWizardPlayerTotals } from "../utils/wizardUtils";
import { getRankColor } from "../utils/skullKingUtils";

export default function WizardHistory() {
  const { user } = useUser();
  const [history, setHistory] = useState<WizardGame[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [expandedGameId, setExpandedGameId] = useState<number | null>(null);

  const fetchData = async () => {
    try {
      const data = await fetchWizardHistory();
      setHistory(data);
    } catch (err) {
    ...
    <div className="container py-4 fade-in" style={{ maxWidth: "1000px" }}>
      <div className="glass-card p-4 shadow-lg">
        <h1 className="fw-bold mb-4">
          🧙 WIZARD <span className="text-accent">HISTORY</span>
        </h1>
        ...
        {history.length === 0 ? (
          ...
          <div className="list-group gap-3">
            {history.map((game) => {
              // game.scores에서 고유 round 값 추출
              const roundsArr = Array.from(
                new Set(game.scores.map((s) => s.round))
              ).sort((a, b) => a - b);
              const completedRounds = new Set(roundsArr);
              const playerTotals = calculateWizardPlayerTotals(
                game.scores,
                completedRounds
